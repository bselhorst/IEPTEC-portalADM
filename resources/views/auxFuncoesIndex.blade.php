                ...
                <ul class="fab-menu-inner">
                    ...
                    <li>
                        <div data-fab-label="Cadastrar">
                            <a href="{{ route('funcoes.create') }}" class="btn btn-light rounded-round btn-icon btn-float">
                    ...
                    <table class="table text-nowrap">
                        ...
                        <tbody>
                            <tr class="table-active table-border-double">
                                <td colspan="1">Lista de Funções</td>
                                <td class="text-right">
                                    <span class="badge bg-blue badge-pill">{{$funcoes->total()}}</span>
                                </td>
                            </tr>
                            @foreach($funcoes as $funcao)
                                <tr>
                                    <td>
                                        <div class="font-weight-semibold">{{ $funcao->funcao }}</div>
                                    </td>
                                    <td class="text-center">
                                        @permission('delete-aux-tecnologia')
                                        <form method="POST" action="{{ route('funcoes.destroy', $funcao->id) }}" onsubmit="return confirm('Deseja deletar esse dado?')">
                                            @csrf
                                            @method('DELETE')
                            ...
                            <tr class="">
                                <td colspan=4>
                                    <ul class="pagination pagination-pager pagination-rounded justify-content-center">
                                        @if ($funcoes->previousPageUrl())
                                            <li class="page-item"><a href="{{ $funcoes->previousPageUrl() }}" class="page-link">← &nbsp; Anterior</a></li>
                                        @endif
                                        @if (!$funcoes->previousPageUrl())
                                            <li class="page-item disabled"><a href="{{ $funcoes->previousPageUrl() }}" class="page-link">← &nbsp; Anterior</a></li>
                                        @endif
                                        @if ($funcoes->nextPageUrl())
                                            <li class="page-item"><a href="{{ $funcoes->nextPageUrl() }}" class="page-link">Próximo &nbsp; →</a></li>
                                        @endif
                                        @if (!$funcoes->nextPageUrl())
                                            <li class="page-item disabled"><a href="{{ $funcoes->nextPageUrl() }}" class="page-link">Próximo &nbsp; →</a></li>
                                        @endif
                                    </ul>
                                </td>
                            </tr>
