...
@section('content')
    ...
    <div class="card">
        <div class="card-body">
            <form class="form-validate-jquery" method="POST" action="{{ (@$dataEdit) ? route('auxsituacaobem.update', $dataEdit->id) : route('auxsituacaobem.store') }}">
                @csrf
                @if(isset($dataEdit))
                    @method('PATCH')
                @endif
                <fieldset class="mb-3">
                    ...
                    <div class="form-group row" style="justify-content: center">
                        @if(isset($dataEdit))
                            <a href="/auxsituacaobem" style="color: red">Você está editando registro, clique aqui para cancelar a edição</a>
                        @endif
                    </div>
                    <div class="form-group row">
                        ...
                        <div class="col-lg-4">
                            <input type="text" name="descricao" class="form-control" placeholder="Ex: Descrição da situação" value="{{ (@$dataEdit) ? $dataEdit->descricao : ''}}">
                        </div>
                    </div>
                </fieldset>
                                        ...
                                        <div class="list-icons">
                                            <div class="list-icons-item dropdown">
                                                ...
                                                <div class="dropdown-menu dropdown-menu-right">
                                                    ...
                                                    <a href="{{ route('auxsituacaobem.edit', $data->id) }}" class="dropdown-item"><i class="icon-pencil7"></i> Editar</a>

                                                    <form method="POST" action="{{ route('auxsituacaobem.destroy', $data->id) }}" onsubmit="return confirm('Deseja deletar esse dado?')">
                                                        @csrf
                                                        @method('DELETE')
